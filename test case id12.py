...
class CancelAddComputer(unittest.TestCase):
...
# navigate to the application home page
    ...
    def test_1_cancel_add_a_new_computer(self):
...
# wait to make sure the new window is loaded
        time.sleep(1)
        self.driver.find_element_by_id("name").send_keys("IBM 1234")
        self.driver.find_element_by_id("introduced").send_keys("2010-02-02")
        self.driver.find_element_by_id("discontinued").send_keys("2015-03-03")
        Select(self.driver.find_element_by_id("company")).select_by_visible_text("IBM")
        time.sleep(1)
        self.driver.find_element_by_link_text("Cancel").click()
#switch back to original window
        self.driver.switch_to_window(self.driver.window_handles[0])
        try: self.is_text_present("Computer database")
